...
pub struct Header {
    ...
    /// Request/Response correlation - required for Request and Response message types
...
impl<'a> Message<'a> {
    ...
    pub fn from_buffer(buffer: &'a [u8]) -> Result<Message<'a>, MessageDeserializeError> {
        ...
        let payload_len_bytes: [u8; 8] = buffer[payload_len_offset..payload_len_offset + PAYLOAD_LEN_SIZE]
            .try_into()
            .map_err(|_| MessageDeserializeError::InvalidHeaderData)?;
        ...
        if buffer.len() < payload_offset + payload_len as usize {
    ...
    /// Parse a Message from a mutable buffer (consuming approach for compatibility)
    pub fn from_buffer_mut(buffer: &mut BytesMut) -> Result<Message<'a>, MessageDeserializeError> {
        ...
        let slice = unsafe {
            // SAFETY: We ensure the buffer has enough bytes and the message structure is valid
    ...
    fn calculate_total_message_len(buffer: &[u8]) -> Result<usize, MessageDeserializeError> {
        ...
        Ok(BASE_HEADER_SIZE + header_len as usize + PAYLOAD_LEN_SIZE + payload_len as usize)
    }
    ...
    pub fn header(&self) -> Result<Header, MessageDeserializeError> {
        rmp_serde::from_slice(self.header_bytes())
            .map_err(|e| MessageDeserializeError::DeserializeError(e))
    }
    ...
    pub fn payload_bytes(&self) -> &[u8] {
        &self.buffer[self.payload_offset..self.payload_offset + self.payload_len as usize]
    }

    /// Deserialize the MessagePack encoded payload into type T (only when needed)
    ...
    pub fn total_size(&self) -> usize {
        BASE_HEADER_SIZE + self.header_len as usize + PAYLOAD_LEN_SIZE + self.payload_len as usize
    }
}
...
mod tests {
    ...
    fn create_valid_message_bytes() -> Vec<u8> {
        ...
        let header = Header { routing: None, reqrep: None };
    ...
    fn test_invalid_version() {
        ...
        assert!(matches!(result, Err(MessageDeserializeError::InvalidVersion)));
    }
    ...
    fn test_incomplete_header() {
        ...
        let result = Message::from_buffer(&buffer);
        assert!(matches!(result, Err(MessageDeserializeError::IncompleteHeader)));
    }
    ...
    fn test_incomplete_payload() {
        ...
        let result = Message::from_buffer(&buffer);
        assert!(matches!(result, Err(MessageDeserializeError::IncompletePayload)));
    }
    ...
    fn test_invalid_header_data() {
        ...
        let result = Message::from_buffer(&buffer);
        assert!(matches!(result, Err(MessageDeserializeError::DeserializeError(_))));
    }
    ...
    fn test_broadcast_with_routing_invalid() {
        ...
        let result = Message::from_buffer(&buffer);
        assert!(matches!(result, Err(MessageDeserializeError::InvalidRoutingData)));
    }
    ...
    fn test_request_without_reqrep_invalid() {
        ...
        let result = Message::from_buffer(&buffer);
        assert!(matches!(result, Err(MessageDeserializeError::InvalidHeaderData)));
    }

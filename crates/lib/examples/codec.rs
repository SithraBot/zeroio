...
use zeroio::{decode::Message, encode::MessageBuilder};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let notif_encoded =
        MessageBuilder::simple_notification(4001, 5001, "/events".to_string(), &notification)?;
    ...
    let raw_encoded =
        MessageBuilder::broadcast(8001).with_raw_payload(binary_data.clone()).build()?;
